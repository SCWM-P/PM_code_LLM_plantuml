...
class Activity:
    ...
    start_node: Node = field(default_factory=lambda: Node(-1), compare=False)
    end_node: Node = field(default_factory=lambda: Node(-1), compare=False)
...
class NodeDict(dict):
    def to_key(self, id_set: Set[str]) -> str:
        return "-".join(sorted(id_set))

    def to_set(self, id: str) -> Set[str]:
        return set(id.split("-"))

    def __setitem__(self, key: Set[str], value: Node) -> None:
        value.start_dependencies = key
        super().__setitem__(self.to_key(key), value)

    def __getitem__(self, key: Set[str]) -> Node:
        return super().__getitem__(self.to_key(key))

    def __delitem__(self, key: Set[str]) -> None:
        return super().__delitem__(self.to_key(key))

    def __contains__(self, key: Set[str]) -> bool:
